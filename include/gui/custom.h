	...
	if ( ( dfd = opendir( dir_name ) ) != NULL ) {
		while ( ( dp = readdir( dfd ) ) != NULL ) {
			...
			if ( ( stbuf.st_mode & S_IFMT ) == S_IFDIR )
				...
				vector_add( &gui_configs_list, sds_config_name );
			}
...
static void gui_subtab( const char* name, const char* subtitle, int* selected ) {
	...
	if ( gui_just_refreshed_subtabs ) {
		vector_add( &gui_subtabs_list, sdscat( sdsnew( name ), "##Subtab" ) );
		vector_add( &gui_subtabs_subtitles_list, sdscat( sdscat( sdsnew( subtitle ), "##SubtabSubtitle" ), name ) );
	}
