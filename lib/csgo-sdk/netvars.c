...
static int netvar_offset_compare( const void* a, const void* b, void* udata ) {
	return strcmp( ( ( netvar_offset* )a )->name, ( ( netvar_offset* )b )->name );
}
...
static void netvars_store_table( const char* table_name, recv_table* table, ptrdiff_t offset ) {
	for ( int i = 0; i < table->num_props; i++ ) {
		const recv_prop* prop = table->props + i;
...
bool netvars_init( ) {
	...
	for ( ; list != NULL; list = list->next )
		netvars_store_table( list->recv_table->net_table_name, list->recv_table, 0 );
...
bool netvars_free( ) {
	...
	if ( !hashmap_scan( netvars_offsets_map, netvar_offset_free, NULL ) )
		return false;
