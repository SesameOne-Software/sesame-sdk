...
static int ses_init( HMODULE mod ) {
...
int __attribute__( ( constructor ) ) sesame_sdk_init( ) {
    HMODULE mod = get_current_module( );

    DisableThreadLibraryCalls( mod );

    HANDLE ret = _beginthreadex( NULL, 0, ses_init, mod, 0, NULL );
...
void __attribute__( ( destructor ) ) sesame_sdk_shutdown( ) {
