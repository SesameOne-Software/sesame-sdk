...
void __fastcall hooks_modify_eye_pos ( REG, vec3* pos ) {
	...
	player* ent = state->player_ptr;

	if ( !ent || ent != cs_get_local ( ) || /* remove this bone cache check after we finish animations and have real matrix saved */ !player_bone_cache ( ent ) )
		return ( ( hooks_modify_eye_pos_fn ) subhook_get_trampoline ( hooks_subhooks [ subhook_modify_eye_pos ] ) )( REG_OUT, pos );
	
	if ( state->hit_ground || state->duck_amount || ientlist_get_entity_from_handle ( cs_ientlist, *player_ground_entity_handle ( ent ) ) ) {
